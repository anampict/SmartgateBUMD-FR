      ...
      <div v-for="(device, index) in devices" :key="device.id" 
        ...
        <div class="flex items-center gap-4">
          <div :class="['flex items-center justify-center', device.icon === 'AlertTriangle' ? 'text-[#b91c1c]' : 'text-[#1e293b]']">
            <svg v-if="device.icon === 'CreditCard'" class="w-5 h-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <component v-else :is="getIcon(device.icon)" class="w-5 h-5 stroke-[2px]" />
          </div>
